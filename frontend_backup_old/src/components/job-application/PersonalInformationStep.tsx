...
const states = [
  ...
  // International (Common Countries/Regions)
  'MX', // Mexico
  'CA', // Canada (provinces will use 2-letter codes)
  'ES', // Spain
  'PR', // Puerto Rico
  ...
  'OTHER' // For other international locations
]

const stateLabels: Record<string, string> = {
  // US States (abbreviated for space)
  ...
  // International
  'MX': 'Mexico', 'ES': 'Spain', 'PR': 'Puerto Rico', 'VI': 'US Virgin Islands',
...
export default function PersonalInformationStep({
  ...
  const validationRules: ValidationRule[] = [
    ...
    { field: 'state', required: true, type: 'string', minLength: 2, maxLength: 5 },
  ...
  const formatPhoneNumber = (value: string) => {
    ...
    // If it starts with a country code (not 1 for US), keep it as international format
    if (numbers.length > 10 && !numbers.startsWith('1')) {
      // International format - just add + prefix
      return `+${numbers}`
    }

    // If starts with 1 and has 11 digits, treat as US with country code
    ...
    // US domestic format (10 digits or less)
    if (numbers.length <= 3) return numbers
    if (numbers.length <= 6) return `(${numbers.slice(0, 3)}) ${numbers.slice(3)}`
    if (numbers.length <= 10) return `(${numbers.slice(0, 3)}) ${numbers.slice(3, 6)}-${numbers.slice(6, 10)}`

    // For numbers longer than 10 digits without country code identifier,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="alternate_phone">{t('jobApplication.steps.personalInfo.fields.alternatePhone')}</Label>
            <div className="relative">
              <Phone className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
              <Input
                id="alternate_phone"
            ...
            <div className="flex gap-4 mt-2">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="alternate_phone_cell" className="text-sm font-normal cursor-pointer">{t('jobApplication.steps.personalInfo.fields.phoneType.cell')}</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                ...
                />
                <Label htmlFor="alternate_phone_home" className="text-sm font-normal cursor-pointer">{t('jobApplication.steps.personalInfo.fields.phoneType.home')}</Label>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className={getError('state') ? 'border-red-500' : ''}>
                  <SelectValue placeholder={t('jobApplication.steps.personalInfo.fields.selectState')} />
                </SelectTrigger>
                <SelectContent>
                  {states.map(state => (
            ...
            <div className="space-y-2">
              <Label htmlFor="country">{t('jobApplication.steps.personalInfo.fields.country')}</Label>
              <Select
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <p className="text-sm text-blue-800">
          <strong>{t('common.note')}:</strong> {t('jobApplication.steps.personalInfo.fields.confidentialityNote')}
        </p>
      </div>
    </div>
  )
