...
export class FormValidator {
  ...
  private translationFunc: ((key: string, options?: any) => string) | null = null;
  ...
  // Set translation function for i18n support
  setTranslationFunction(t: (key: string, options?: any) => string): void {
    this.translationFunc = t;
  }

  // Get translated message or fallback to key/default
  private getTranslatedMessage(key: string, fallback?: string, options?: any): string {
    if (this.translationFunc) {
      const translated = this.translationFunc(key, options);
  ...
  private readonly patterns = {
    email: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
    phone: /^(\+?1[-.\s]?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}|\+[1-9][0-9]{0,3}[-.\s]?[0-9]{1,4}[-.\s]?[0-9]{1,4}[-.\s]?[0-9]{1,9}|\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}|[0-9]{10,15})$/,
    ssn: /^\d{3}-\d{2}-\d{4}$/,
    zipCode: /^\d{5}(-\d{4})?$/,
    url: /^https?:\/\/.+/
  };
  ...
  private readonly messages = {
    ...
    phone: 'Please enter a valid phone number (e.g., +1 555 123 4567, +52 55 1234 5678, or (555) 123-4567)',
  ...
  validateField(value: any, rule: ValidationRule, formData?: any): string | null {
    ...
    if (rule.required && this.isEmpty(value)) {
      if (rule.message) {
        // If message starts with 'validation.', treat it as a translation key
        if (rule.message.startsWith('validation.')) {
          return this.getTranslatedMessage(rule.message, this.messages.required);
        }
        return rule.message;
      }
      return this.getTranslatedMessage('validation.common.required', this.messages.required);
    }
    ...
    if (rule.type && !this.validateType(value, rule.type)) {
      if (rule.message) {
        if (rule.message.startsWith('validation.')) {
          return this.getTranslatedMessage(rule.message, (this.messages as any)[rule.type]);
        }
        return rule.message;
      }
      const typeKey = `validation.common.invalid${rule.type.charAt(0).toUpperCase() + rule.type.slice(1)}`;
      return this.getTranslatedMessage(typeKey, (this.messages as any)[rule.type] || `Invalid ${rule.type}`);
    }

    // Length validation for strings
    if (typeof value === 'string') {
      if (rule.minLength && value.length < rule.minLength) {
        if (rule.message && !rule.message.startsWith('validation.')) {
          return rule.message;
        }
        return this.getTranslatedMessage(
          'validation.common.minLength',
          this.messages.minLength.replace('{min}', rule.minLength.toString()),
          { min: rule.minLength }
        );
      }
      if (rule.maxLength && value.length > rule.maxLength) {
        ...
        return this.getTranslatedMessage(
          'validation.common.maxLength',
          this.messages.maxLength.replace('{max}', rule.maxLength.toString()),
          { max: rule.maxLength }
        );
      }
    }
    ...
    if (rule.pattern && typeof value === 'string' && !rule.pattern.test(value)) {
      if (rule.message) {
        if (rule.message.startsWith('validation.')) {
          return this.getTranslatedMessage(rule.message, `Invalid format for ${rule.field}`);
        }
        return rule.message;
      }
      return this.getTranslatedMessage('validation.common.invalidFormat', `Invalid format for ${rule.field}`);
    }

    // Custom validation
    if (rule.customValidator) {
      const result = rule.customValidator(value, formData);
      // If custom validator returns a translation key, translate it
      if (result && result.startsWith('validation.')) {
        return this.getTranslatedMessage(result, result);
      }
      return result;
    }
  ...
  private formatPhone(value: string): string {
    ...
    if (numbers.startsWith('1') && numbers.length === 11) {
      const usNumber = numbers.slice(1);
...
export const ValidationRules = {
  ...
  directDeposit: [
    ...
    { field: 'primaryAccount.routingNumber', required: true, type: 'string' as const, pattern: /^\d{9}$/, message: 'validation.directDeposit.routingNumberFormat' },
    ...
    { field: 'authorizeDeposit', required: true, customValidator: (value: any) => value === true ? null : 'validation.forms.authorizationRequired' }
  ],
